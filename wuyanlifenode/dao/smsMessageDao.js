...
/**
 * 根据手机号查询短信 验证时间小于五分钟的
 * @param {*} mobile 
 */
SmsMessageDao.prototype.getMessageByMobile = async function(mobile) {
	var startTime = Math.floor((new Date()).getTime() / 1000) - 10*60 * 60;
	try {
		var result = await knex('wx_message').where('verifyTime', '>', startTime).andWhere("telephone", mobile).select('*');
		if(result.length > 0) {
			return result;
		}
	} catch(err) {
