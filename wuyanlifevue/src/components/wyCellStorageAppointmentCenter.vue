<template lang="html">
	<div class="tf-container" ref='tfContainer'>
		...
		<tfHeader title="細胞存儲预约" :show-goback="false" :show-center="true"></tfHeader>
		<div>{{msgs}}</div>
		<div>{{msg}}</div>
		<div id="mycards" style="height:800px;overflow-y:auto;">
			<div id="tool">
				<span style="color:black;font-size:16px;float:right;font-style:italic;">
					<a href="javascript:void(0);" @click="addCellStorageAppoinment($event);">{{addAppointment}}</a>
				</span>
			</div>
			<div class="add-topic-tab" v-for="appointment in appointments" style="width:100%;">
				<router-link :to="{ name: 'wyCellStorageAppointmentDetail', params: {id : appointment.id} }" style="width:100%;">
					<div style="width:100%; padding:10px;border:1px solid darksalmon;border-radius: 5px;text-align:left;">
						<span style="color:black;font-size:16px;font-weight:bold;">存储项目：{{appointment.cellStorageProject}}</span><br />
...
<script>
	...
	import wyCellStorageAppointmentAdd from './wyCellStorageAppointmentAdd.vue'
	...
	export default {
		...
		methods: {
			...
			login() {
				window.localStorage.setItem("wxOpenId",'1') //todo
				...
				if(!!wxCode) {
					...
					if(!!wxOpenId) {
						//获取列表数据
						var url = "http://localhost:8081/loadCellStorageAppointmentList";
						this.$post(url, {
							wxOpenId: wxOpenId
						}).then((data) => {
			...
			addCellStorageAppoinment: function(envent) {
				this.$router.push({
